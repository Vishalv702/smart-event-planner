...
class EventController {
  ...
  async getEvents(req, res) {
    try {
      const { search, fromDate, toDate, limit = 6, page = 1 } = req.query;
  ...
  async checkEventWeather(req, res) {
    try {
      ...
      //   try {
      //     const pastData = await weatherService.getHistoricalWeather(
      //       coords.lat,
      //       coords.lon,
      //       pastDate
      //     );
      ...
      // Hourly Forecast
      // for now, it's commented out as it not required in the current implementation 
      // let hourlyForecast = [];
      // try {
      ...
      //     hourlyForecast = await weatherService.getHourlyForecast(
      //       coords.lat,
      //       coords.lon,
      //       event.date
      //     );
...
export const updateEventWeatherData = async (
  event,
  weatherData,
  suitability,
  extra = {}
